document.addEventListener('DOMContentLoaded', function() {
    ...
    function initScrollAnimations() {
        const animatedElements = document.querySelectorAll('.service-card, .benefit-card, .step, .achievement-item, .faq-item');
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (entry.target.parentElement.children) {
    ...
    enhanceVideoAnimations();
    
    // Add animation to CTA button
    const ctaButton = document.querySelector('.cta .btn');
    if (ctaButton) {
        setInterval(() => {
            ctaButton.classList.add('animate-pulse-slow');
        }, 4000);
    }
    
    // Animate language switcher on load
    setTimeout(() => {
        const languageSwitcher = document.querySelector('.language-switcher');
        if (languageSwitcher) {
            languageSwitcher.style.opacity = '0';
            languageSwitcher.style.transform = 'translateX(20px)';
            languageSwitcher.style.transition = 'all 0.5s ease-out';
    ...
    // Achievement image hover effects
    ...
    function addLoadingAnimation(button) {
        ...
        button.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' + getTranslation('form.sending');
    ...
    // Enhanced form submissions with loading animations
    if (contactForm) {
        contactForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            
            const submitBtn = this.querySelector('button[type="submit"]');
            const originalText = addLoadingAnimation(submitBtn);
            
            // Your existing form submission logic here...
            // Remember to call removeLoadingAnimation(submitBtn, originalText) when done
        });
    }
    ...
    // Console log for debugging animations
